...
Depois de um segundo teste, mais três ajustes focados em facilitar o disparo, especialmente para uma criança jogando em toque: a mira deixou de ficar fixa no centro da tela e agora acompanha o ponto real de mira — a posição do mouse (onde o mouse já era a origem real do disparo) ou o centro da câmera em toque/gamepad (onde não há cursor e a mira sempre foi pelo centro mesmo); o Puffador ganhou um **botão de disparo dedicado** (`FireButtonView`) no canto inferior direito da tela, útil principalmente em toque, onde tocar o mundo para atirar competia com o gesto de arrastar para olhar ao redor; e o Puffador agora é **equipado automaticamente** assim que concedido a cada participante (via `Humanoid:EquipTool`), sem precisar abrir o inventário e clicar nele antes de poder jogar.

Cada participante recebe, ao entrar em `Active`, o **Puffador**: uma ferramenta cartunesca construída inteiramente com instâncias nativas (sem assets externos), já equipada na mão. O jogador mira e dispara com mouse, toque, gamepad ou o botão de disparo dedicado — todos convergem para a mesma função no cliente, que só calcula a direção de mira e envia um único `Vector3` normalizado pelo `RemoteEvent RequestPuff`. O servidor é a única autoridade: valida participação, elegibilidade, cadência e origem antes de simular o projétil por raycast determinístico.
